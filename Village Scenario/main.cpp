...
float sun_current_position = 0.0; // variable to store current position
float sun_target_position = 0.0; // variable to store target position
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    case 'n':
        ...
        isNightMode = true;
        isSunMoving = false;
        sun_target_position = -0.95;
    sun_current_position = sun_move;
        playSound();
        break;
    case 'd':
        start2 = true;
...
{
    glBegin(GL_POLYGON);
    glColor3ub(0, 0, 0); // dark green color
    glVertex2f(0.4f, -0.58f);
    glVertex2f(0.39f, -0.5f);
    glVertex2f(0.42f, -0.58f);
    glVertex2f(0.43f, -0.44f);
...
void display2()
{
    ...
    {
        //sun_move -= 0.011;
         if (isSunMoving) { // check if sun should move
            sun_move -= 0.011;
        }
        if(sun_move < -0.95)
        {
    ...
    if (start1 == true)
    {
            if (!isSunMoving)
    {
        // Move sun from current position to target position
        float delta = (sun_target_position - sun_current_position) * 0.01; // calculate delta value
        sun_current_position += delta; // update current position
        sun_move = sun_current_position; // set new position of sun

        // If sun has reached the target position, stop moving it
        if (sun_current_position <= sun_target_position)
        {
            sun_move = sun_target_position;
            isSunMoving = true;
            start1 = false;
            start3 = true;
            glDisable(GL_LIGHT0);
